...
//-------------------------------------------------------------------
// DsG4ScintSimple is a class modified from G4Scintillation
...
// DsG4ScintSimple::operator=(const DsG4ScintSimple &right)
// {
// }
...
DsG4ScintSimple::DsG4ScintSimple(const G4String& processName,
                                     G4ProcessType type)
...
DsG4ScintSimple::~DsG4ScintSimple() 
{
...
//
G4VParticleChange*
DsG4ScintSimple::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)

// This routine simply calls the equivalent PostStepDoIt since all the
// necessary information resides in aStep.GetTotalEnergyDeposit()

{
    return DsG4ScintSimple::PostStepDoIt(aTrack, aStep);
}
...
//
G4VParticleChange*
DsG4ScintSimple::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
    ...
    if (flagReemission) {   
       ...
       if(FastMu300nsTrick)  {
        ...
        if (verboseLevel > 0) {
            G4cout << " ScintillationYield: " << ScintillationYield
                   << " QuenchedTotalEnergyDeposit: " << QuenchedTotalEnergyDeposit
                   << std::endl;
        }
...
void DsG4ScintSimple::BuildThePhysicsTable()
{
...
G4double DsG4ScintSimple::GetMeanFreePath(const G4Track&,
                                            G4double ,
...
G4double DsG4ScintSimple::GetMeanLifeTime(const G4Track&,
                                            G4ForceCondition* condition)
{
    *condition = Forced;
...
G4bool DsG4ScintSimple::IsApplicable(const G4ParticleDefinition& aParticleType)
{
        std::cout << "DsG4ScintSimple::IsApplicable " << aParticleType.GetParticleName() << std::endl;
        // skip optical photon.
